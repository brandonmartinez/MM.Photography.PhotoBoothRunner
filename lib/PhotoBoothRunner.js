...
exports.start = function (options, server) {
		...
		if(!lock){
			...
			.then(function() {
				...
				return exec(captureCommand, {}).then(function(stdout){
					server.photoCaptured('somewhere.jpg');
					debug(stdout);
				});
			})
			.done(function() {
	...
	configuration = {
		ShutterButtonPin: options.ShutterButtonPin || 22,
		ShutterButtonLedPin: options.ShutterButtonLedPin || 18,
		QuitButtonPin: options.QuitButtonPin || 4,
		ProcessingLedPin: options.ProcessingLedPin || 24,
		PosingLedPin: options.PosingLedPin || 23,
		HeartbeatLedPin: options.HeartbeatLedPin || 25,
		// fswebcam -r 1280x720 --no-banner "/home/pi/Pictures/Photobooth/%Y%m%d%H%M%S.jpg"
		PhotoCapturer: options.PhotoCapturer || 'rpicam',
	...
	switch(configuration.PhotoCapturer.toLowerCase()) {
		...
		case 'fswebcam':
			...
			break;
		case 'rpicam':
			captureCommand = 'raspistill -o "' + configuration.CapturedPhotoFilenameTemplate + '"';
			break;
		default:
	...
	process.on('SIGINT', function() {
		try {
			...
			debug('Error caught while trying to exit.', error);
		}
		gpio.destroy();
	});
